...
const TicketSelector = () => {
    ...
    const screeningData = selectedMovie?.screenings.find((s) => s.id === selectedScreeningId);
    ...
    // Local state for smoother editing
    const [localCounts, setLocalCounts] = useState(ticketCounts);

    useEffect(() => {
        setLocalCounts(ticketCounts); // sync if Redux updates externally
    }, [ticketCounts]);

    const handleChange = (type, value) => {
        // allow empty string for smooth deletion
        if (/^\d*$/.test(value)) {
